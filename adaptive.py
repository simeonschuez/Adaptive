...
class AttentiveCNN(nn.Module):
    def __init__(self, embed_size, hidden_size):
        ...
        # delete the last fc layer and avg pool.
...
class Atten(nn.Module):
    ...
    def forward(self, V, h_t, s_t):
        ...
        alpha_t = torch.softmax(z_t.view(-1, z_t.size(2)),
            dim=1).view(
        ...
        alpha_hat_t = torch.softmax(extended.view(-1, extended.size(2)),
            dim=1).view(
...
class Decoder(nn.Module):
    ...
    def forward(self, V, v_g , captions, states=None):
        ...
        # x_t = [w_t;v_g]
        x = torch.cat((embeddings, v_g.unsqueeze(1).expand_as(embeddings)),
            dim=2)
        ...
        for time_step in range(x.size(1)):
            ...
            hiddens[:, time_step:time_step+1, :] = h_t  # Batch_first
