...
class DSet(data.Dataset):
    ...
    def __init__(
        ...
        if type(self.split == list):
            self.split_df = self.df.loc[self.df.split.isin(self.split)]
        else:
...
class COCOCaptionDataset(DSet):
    ...
    def __getitem__(self, index):
        ...
        img_path = os.path.join(
            ...
            filename_from_id(img_id, prefix='COCO_{coco_split}2014_')
            )
        ...
        return image, target, target_len, img_id, img_path
...
def captions_collate_fn(data):
    ...
    images, captions, lengths, img_ids, filenames = zip(*data)
